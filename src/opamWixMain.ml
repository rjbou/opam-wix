...
let resolve_path (type a) env
  ...
  let expnaded_path = OpamFilter.expand_string env path in
  if not @@ Filename.is_relative expnaded_path
  then F.of_string expnaded_path
  else begin
    OpamConsole.warning
      "Specified in config path %s is relative. Searching in current directory..."
        (OpamConsole.colorise `bold path);
    F.(OpamFilename.cwd () / expnaded_path)
  end
